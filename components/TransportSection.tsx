...
import { Truck, CheckSquare, Square, Plus, Trash2, ChevronUp, ChevronDown, Combine, Info, Unplug, AlertTriangle, Calendar } from 'lucide-react';
...
export const TransportSection: React.FC<Props> = ({ 
  ...
  const checkMergeDates = (supplierName: string) => {
      const suppliersToMerge = suppliers.filter(s => s.name === supplierName && s.isIncluded !== false);
      const dates = new Set(suppliersToMerge.map(s => s.deliveryDate || 'Nieznana'));
      
      if (dates.size > 1) {
          const dateList = Array.from(dates).join(', ');
          setMergeWarning(`UWAGA: Wybrani dostawcy mają różne daty dostawy (${dateList}).\n\nPołączenie transportu sugeruje jedną wspólną dostawę. Czy chcesz kontynuować?`);
      } else {
          setMergeWarning(null);
      }
      setMergeCandidate(supplierName);
  };

  const performMerge = () => {
      if (!mergeCandidate) return;
      
      const supplierName = mergeCandidate;
      const suppliersToMerge = suppliers.filter(s => s.name === supplierName && s.isIncluded !== false);
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 rounded-sm border border-zinc-200 dark:border-zinc-800 mb-6 overflow-hidden transition-colors">
                                        ...
                                        >
                                            ...
                                            <td className={cellClass}>
                                                <SmartInput 
                                                    ...
                                                    onChange={(val) => updateSupplierTransport(supplier.id, { pricePerTruck: val })} 
                                                    disabled={isOrgBySupplier} 
      ...
      <ConfirmDialog 
          ...
          title={`Scalanie transportu: ${mergeCandidate}`}
          message={mergeWarning || `Czy na pewno chcesz połączyć transporty dla "${mergeCandidate}"?`}
          confirmLabel="Scal"
          cancelLabel="Anuluj"
          isDanger={!!mergeWarning}
          onConfirm={performMerge}
          onCancel={() => { setMergeCandidate(null); setMergeWarning(null); }}
      />
    </div>
  );
};
