...
export const TransportSection: React.FC<Props> = ({
    ...
    const headerClass = "p-2 bg-zinc-100/50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 font-bold uppercase text-[10px] tracking-wider text-left first:rounded-tl first:rounded-bl last:rounded-tr last:rounded-br";
    const cellClass = "p-2 border-b border-zinc-50 dark:border-zinc-800/50 text-xs align-middle";
    const inputClass = "w-full p-1.5 bg-zinc-100 dark:bg-zinc-700 border-0 rounded text-right font-bold text-zinc-800 dark:text-white focus:ring-1 focus:ring-inset focus:ring-amber-500 outline-none transition-all text-xs";
    const textInputClass = "w-full p-1.5 bg-transparent border-0 font-medium text-zinc-800 dark:text-zinc-200 placeholder-zinc-400 focus:bg-white focus:shadow-inner rounded transition-colors text-xs";
    const selectClass = "w-full p-1.5 bg-zinc-100 dark:bg-zinc-700 border-0 rounded text-xs font-bold focus:ring-1 focus:ring-amber-500 outline-none cursor-pointer";
        ...
        return (
            <div className="space-y-4">
                {Object.entries(grouped).map(([name, group]) => (
                    <div key={name} className="border border-zinc-200 dark:border-zinc-700 rounded overflow-hidden">
                        ...
                        <div className="divide-y divide-zinc-100 dark:divide-zinc-800 bg-white dark:bg-zinc-900">
                            {group.map(s => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-4 h-4 border mr-3 flex items-center justify-center rounded transition-colors ${isChecked ? 'bg-blue-500 border-blue-500 text-white' : 'border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800'}`}>
    ...
    return (
        <div className="bg-white dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 mb-6 overflow-hidden transition-colors">
            <div
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-3 mb-4 rounded flex flex-wrap gap-4 items-center justify-center border-b border-blue-100 dark:border-blue-900/30">
                                ...
                                {potentialMerges.map(name => (
                                    <button
                                        ...
                                        className="bg-white dark:bg-zinc-800 border border-blue-200 dark:border-blue-700 text-blue-600 dark:text-blue-400 px-3 py-1 rounded text-[10px] font-bold hover:bg-blue-100 dark:hover:bg-blue-900/50 flex items-center gap-1 transition-colors shadow-sm"
                                    >
