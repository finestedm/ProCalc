                                ...
                                <div className="p-1">
                                    {group.map((p, i) => (
                                        <div key={i} className={i > 0 ? "mt-2 pt-2 border-t border-zinc-100" : ""}>
                                            <div className="font-bold text-blue-600">{p.projectNumber}</div>
                                            <div className="text-[10px] text-zinc-500">{p.customerName}</div>
                                            <div className="text-[11px] mt-1 font-semibold">Dostawa: {p.deliveryDate || 'Brak daty'}</div>
                                            <div className="text-[9px] text-zinc-400 mt-1">Dostawcy: {p.suppliers.join(', ')}</div>
                                        </div>
                                    ))}
                                </div>
                            </Tooltip>
                                ...
                                <div className="max-w-[200px]">
                                    {group.map((p, i) => (
                                        <div key={i} className={i > 0 ? "mt-3 pt-3 border-t border-zinc-200" : ""}>
                                            <h3 className="font-bold text-zinc-900 m-0">{p.projectNumber}</h3>
                                            <p className="text-sm text-zinc-600 m-0">{p.customerName}</p>
                                            <div className="mt-2 text-xs">
                                                <span className="font-semibold text-zinc-500">Adres:</span>
                                                <div className="text-zinc-800">{p.address}</div>
                                            </div>
                                            <div className="mt-1 text-xs">
                                                <span className="font-semibold text-zinc-500">Planowana dostawa:</span>
                                                <div className="text-blue-600 font-bold">{p.deliveryDate}</div>
                                            </div>
                                            <div className="mt-1 text-xs">
