  ...
  return (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-140px)]">
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-sm shadow-sm flex flex-col overflow-hidden h-full">
            ...
            <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-sm shadow-sm flex flex-col overflow-hidden h-full">
                ...
                <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                    <form onSubmit={addTask} className="flex gap-2">
                        <input 
                            type="text" 
                            className="flex-1 px-3 py-2 border border-zinc-200 dark:border-zinc-700 rounded-sm text-sm bg-white dark:bg-zinc-900 focus:border-blue-400 outline-none transition-colors placeholder-zinc-400"
                        ...
                        <input 
                            type="date" 
                            className="w-32 px-2 py-2 border border-zinc-200 dark:border-zinc-700 rounded-sm text-xs bg-white dark:bg-zinc-900 focus:border-blue-400 outline-none"
                            value={newTaskDate}
                        ...
                        <button 
                            ...
                            className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 py-2 rounded-sm transition-colors flex items-center justify-center"
                        >
                            <Plus size={18}/>
                        </button>
                    </form>
                    ...
                    {(data.tasks || []).length === 0 ? (
                        ...
                        <ul className="space-y-2">
                            {(data.tasks || []).map(task => (
                                <li 
                                    key={task.id} 
                                    className={`group flex flex-col p-3 bg-white dark:bg-zinc-900 border rounded-sm transition-all hover:shadow-sm ${
                                        task.isCompleted 
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button 
                                            ...
                                            className={`shrink-0 transition-colors ${
                                                task.isCompleted ? 'text-green-500' : 'text-zinc-300 hover:text-blue-500'
                                            }`}
                                        >
                                        ...
                                        <button 
                                            ...
                                            className="text-zinc-300 hover:text-red-500 p-1.5 rounded-sm hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all"
                                            title="Usuń zadanie"
                                        >
                                            <Trash2 size={16} />
                                        </button>
